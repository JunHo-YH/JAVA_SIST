 ...
 * ===============
 * 변수 => 배열     |
 * 명령문 => 메서드	 |          => 클래스 (여러 개를 다시 묶는다(관련된 클래스끼리) => package
 * ====		     |
 * 연산자, 제어문	 |
 * ==============|
 */
public class 메서드활용_4 {
	static String[] title={
            "Your Lights",
			"겨울잠",
			"우린 어쩌다 헤어진 걸까",
	...
	public static void main(String[] args) {
		...
		*/
		// 2. 찾기
			...
			for (int i = 0; i < 50; i++) {
				if (title[i].contains(ss)) { // 문자열에 포함 여부 확인 => boolean => 오라클 ( LIKE ), REGXP_LIKE(), 정규식
					System.out.println("순위: " + (i + 1));
					System.out.println("노래명: " + title[i]);
					System.out.println("가수명: " + singer[i]);
					System.out.println("앨범: " + album[i]);
